module.exports = (printers) => {
    ...
    let inkData = {};
    let colorArray = [];
    let colors_info = [{name:'cyan',    number:4, max_capacity_bw:3, max_capacity_color: 10},
                       {name:'magenta', number:6, max_capacity_bw:3, max_capacity_color: 11},
                       {name:'yellow',  number:8, max_capacity_bw:3, max_capacity_color: 12}];
    let black_and_white_loop_info = [
                       {name :'black',  number:2, max_capacity_bw:3, max_capacity_color: 9}];
    ...
    let snmpAdresses =[
        ...
        // {ip:"192.168.66.26",  color: true,  name:'pr-tln-6k-HPM476dn',     key:'pr_tln_6k_HPM476dn__',    max_capacity: ? },        //pr-tln-6k-HPM476dn
        ...
        {ip: "192.168.67.3",    color: false,  name: 'pr-tln-4k-HP_P3005',    key: 'pr_tln_4k_HP_P3005__',   max_capacity: 'noidea'},        //pr-tln-4k-HP_P3005
        {ip: "192.168.66.11",   color: false,  name: 'pr-tln-4k-HP521dn',     key: 'pr_tln_4k_HP521dn__',    max_capacity:false},        //pr-tln-4k-HP521dn
        {ip: "192.168.66.187",  color: true,   name: 'pr-tln-3k-HP476dn',     key: 'pr_tln_3k_HP476dn__',    max_capacity:'unreachable printer'},        //pr-tln-3k-HP476dn
        //{ip:"192.168.71.10",  color: false,  name:'pr-tln-k3-k2-HPM605',    key:'pr_tln_3k_2k_HPM605__',   max_capacity: ? },        //pr-tln-k3-k2-HPM605
            ...
            if (error) {
                ...
                if (snmpAdresses[i].color === true && snmpAdresses[i].max_capacity === false) {
                    for (let x = 0; x < colors_loop_info.length; x++) {
                        ...
                        inkData[snmpAdresses[i].key + printer_name.name] = varbinds[printer_name.number].value;
                        // console.log(snmpAdresses[i].key+printer_name.name, varbinds[printer_name.number].value);
                    }
                    ...
                    for (let x = 0; x < black_and_white_loop_info.length; x++) {
                        let printer_name = black_and_white_loop_info[x];
                        let inc_precentage = Math.round((varbinds[printer_name.number].value / varbinds[printer_name.max_capacity_bw].value) * 100);
                        inkData[snmpAdresses[i].key + printer_name.name] = inc_precentage;
                    ...
                    for (let x = 0; x < colors_loop_info.length; x++) {
                        let printer_name = colors_loop_info[x];
                        let inc_precentage = Math.round((varbinds[printer_name.number].value / varbinds[colors_loop_info[x].max_capacity_color].value) * 100);
                        inkData[snmpAdresses[i].key + printer_name.name] = inc_precentage;
                        //console.log(snmpAdresses[i].key+printer_name.name,inc_precentage);
                    }
                }
